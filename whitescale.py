...
def process_svg(input_svg_path, output_subfolder, scale_factor, category_name):
    ...
    for loop_index, shape_id in enumerate(shape_ids, start=1):
        ...
        filename = f"{category_name}_shape{loop_index}_{svg_name}.png"
...
def scale_png(png_filepath):
    with Image.open(png_filepath) as img:
        # Resize the image first (same as before)
        width, height = img.size
        target_size = 2000
        ...
        # Now force every visible pixel to solid white
        if img.mode == 'RGBA':
            # Split into RGB + alpha
            r, g, b, a = img.split()
            # Create a new white background (all 255) with the same size
            white_rgb = Image.new("RGB", img.size, (255, 255, 255))
            # Re‐attach alpha channel so the shape remains but is pure white
            img = Image.merge("RGBA", (*white_rgb.split(), a))
        else:
            # If there’s no alpha, just fill with white
            white = Image.new("RGB", img.size, (255, 255, 255))
            # Convert white to RGBA so that downstream code expecting RGBA still works
